...
class Rectangle extends Component {
  ...
  updateBottomRow() {
    ...
    for (let i = 0; i < newRectangle[1].length; i++) {
      ...
      console.log(i);
      if (i === 0 || i === 3) {
        newNumber = this.getRandomInt(2, 4);
      } else {
        newNumber = this.getRandomInt(4, 8);
      }
    ...
    const rectangleDivs = rectangle.map((row, rowIndex) => (
      <div className="row" key={rowIndex}>
        {row.map((square, squareIndex) => {
          ...
          const isBottomMiddleSquare =
            rowIndex === 1 && (squareIndex === 1 || squareIndex === 2);
          const isBottomRightSquare = rowIndex === 1 && squareIndex === 3;

          const squareClass = isQBSquare
            ? "qb-square"
            : isRBSquare
            ? "rb-square"
            : isWRSquare
